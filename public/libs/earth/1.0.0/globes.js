...
function currentPosition() {
  const lambda = micro.floorMod(new Date().getTimezoneOffset() / 4, 360); // 24 hours * 60 min / 4 === 360 degrees
  return [lambda, 0];
}
  ...
  return {
    ...
    orientation: function (o, view) {
      ...
      if (micro.isValue(o)) {
        ...
        projection.rotate(
          _.isFinite(lambda) && _.isFinite(φ)
            ? [-lambda, -φ, rotate[2]]
            : this.newProjection(view).rotate()
        );
        projection.scale(
          _.isFinite(scale)
            ? micro.clamp(scale, extent[0], extent[1])
            : this.fit(view)
        );
