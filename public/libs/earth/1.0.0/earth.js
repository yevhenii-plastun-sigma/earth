...
function buildMesh(resource) {
  ...
  return micro.loadJson(resource).then(function (topo) {
    if (cancel.requested) return null;
    log.time("building meshes");
    let o = topo.objects;
    let coastLo = topojson.feature(
      topo,
      micro.isMobile() ? o.coastline_tiny : o.coastline_110m
    );
    let coastHi = topojson.feature(
      topo,
      micro.isMobile() ? o.coastline_110m : o.coastline_50m
    );
    let lakesLo = topojson.feature(
      topo,
      micro.isMobile() ? o.lakes_tiny : o.lakes_110m
    );
    let lakesHi = topojson.feature(
      topo,
      micro.isMobile() ? o.lakes_110m : o.lakes_50m
    );
...
function buildGrids() {
  ...
  downloadsInProgress++;
  let loaded = products
    .productsFor(configuration.attributes)
    .map(async (product) => {
...
function distort(projection, λ, φ, x, y, scale, wind) {
  let u = wind[0] * scale;
  let v = wind[1] * scale;
  let d = micro.distortion(projection, λ, φ, x, y);
...
function animate(globe, field, grids) {
  ...
  let colorStyles = micro.windIntensityColorScale(
...
function drawOverlay(field, overlayType) {
  ...
  let ctx = d3.select("#overlay").node().getContext("2d"),
    grid = (gridAgent.value() || {}).overlayGrid;

  micro.clearCanvas(d3.select("#overlay").node());
  micro.clearCanvas(d3.select("#scale").node());
  if (overlayType) {
    if (overlayType !== "off") {
  ...
  if (grid) {
    ...
    let c = colorBar.node(),
      ...
      n = c.width - 1;
    for (let i = 0; i <= n; i++) {
      let rgb = scale.gradient(micro.spread(i / n, bounds[0], bounds[1]), 1);
    ...
    colorBar.on("mousemove", function () {
      ...
      let pct = micro.clamp((Math.round(x) - 2) / (n - 2), 0, 1);
      let value = micro.spread(pct, bounds[0], bounds[1]);
      let elementId =
        grid.type === "wind" ? "#location-wind-units" : "#location-value-units";
      let units = createUnitToggle(elementId, grid).value();
      colorBar.attr(
        "title",
        micro.formatScalar(value, units) + " " + units.label
      );
    });
  }
}
...
function showDate(grids) {
  let date = new Date(validityDate(grids)),
    isLocal = d3.select("#data-date").classed("local");
  let formatted = isLocal ? micro.toLocalISO(date) : micro.toUTCISO(date);
...
function showGridDetails(grids) {
  ...
  if (grids) {
    ...
    let pd = grids.primaryGrid.description(langCode),
...
function showWindAtLocation(wind, product) {
  let unitToggle = createUnitToggle("#location-wind-units", product),
    units = unitToggle.value();
  d3.select("#location-wind").text(micro.formatVector(wind, units));
...
function showOverlayValueAtLocation(value, product) {
  let unitToggle = createUnitToggle("#location-value-units", product),
    units = unitToggle.value();
  d3.select("#location-value").text(micro.formatScalar(value, units));
...
function showLocationDetails(point, coord) {
  ...
  if (_.isFinite(λ) && _.isFinite(φ)) {
    d3.select("#location-coord").text(micro.formatCoordinates(λ, φ));
...
function init() {
  ...
  d3.select("#sponsor-link")
    .attr("target", micro.isEmbeddedInIFrame() ? "_new" : null)
...
Promise.resolve(true).then(init).then(start).catch(report.error);
