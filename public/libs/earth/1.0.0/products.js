...
const products = (function () {
  ...
  let catalogs = {
    ...
    oscar: micro.loadJson([OSCAR_PATH, "catalog.json"].join("/")),
  };
    ...
    return Object.assign(
      {
        ...
        load: function (cancel) {
          let me = this;
          return Promise.all(this.paths.map(micro.loadJson)).then(function (
            files
          ) {
  ...
  function gfs1p0degPath(attr, type, surface, level) {
    ...
    let file =
      [stamp, type, surface, level, "gfs", "1.0"]
        .filter(micro.isValue)
        .join("-") + ".json";
  ...
  function gfsStep(date, step) {
    let offset = (step > 1 ? 8 : step < -1 ? -8 : step) * 3,
  ...
  function describeSurface(attr) {
    return attr.surface === "surface"
      ? "Surface"
      : micro.capitalize(attr.level);
  }

  function describeSurfaceJa(attr) {
    return attr.surface === "surface" ? "地上" : micro.capitalize(attr.level);
  }
  ...
  let FACTORIES = {
        ...
        return buildProduct({
          ...
          scale: {
            ...
            gradient: function (v, a) {
              return micro.extendedSinebowColor(Math.min(v, 100) / 100, a);
            },
          },
    ...
    temp: {
      ...
      create: function (attr) {
        return buildProduct({
          ...
          scale: {
            bounds: [193, 328],
            gradient: micro.segmentedColorScale([
              [193, [37, 4, 42]],
              [206, [41, 10, 130]],
              [219, [81, 40, 40]],
      ...
      create: function (attr) {
        return buildProduct({
          ...
          scale: {
            bounds: [0, 100],
            gradient: function (v, a) {
              return micro.sinebowColor(Math.min(v, 100) / 100, a);
            },
          },
        });
      },
    ...
    air_density: {
      ...
      create: function (attr) {
        return buildProduct({
          ...
          builder: function (file) {
            let vars = file.variables;
            let air_density = vars.air_density,
          ...
          scale: {
            bounds: [0, 1.5],
            gradient: function (v, a) {
              return micro.sinebowColor(Math.min(v, 1.5) / 1.5, a);
            },
          },
        });
      },
    ...
    wind_power_density: {
      ...
      create: function (attr) {
        let windProduct = FACTORIES.wind.create(attr);
        let airdensProduct = FACTORIES.air_density.create(attr);
        return buildProduct({
          field: "scalar",
          ...
          builder: function (windFile, airdensFile) {
            let windBuilder = windProduct.builder(windFile);
            let airdensBuilder = airdensProduct.builder(airdensFile);
            let windData = windBuilder.data,
              windInterpolate = windBuilder.interpolate;
            let airdensData = airdensBuilder.data,
            ...
            builder: function (file) {
              ...
              return {
                ...
                data: function (i) {
                  let u = uData[i],
                    v = vData[i];
                  return micro.isValue(u) && micro.isValue(v) ? [u, v] : null;
                },
              };
            },
            units: [
  ...
  function lookupOscar(catalog, date, offset) {
    ...
    let prefix = micro.ymdRedelimit(date, "/", ""),
  ...
  function oscarDate(catalog, attr) {
    let file = lookupOscar(catalog, attr.date);
    let parts = file ? micro.ymdRedelimit(file, "", "/").split("/") : null;
    return parts
  ...
  function oscarStep(catalog, date, step) {
    let file = lookupOscar(
      catalog,
      micro.dateToUTCymd(date, "/"),
  ...
  function buildGrid(builder) {
    ...
    function interpolate(λ, φ) {
      let i = micro.floorMod(λ - λ0, 360) / Δλ; // calculate longitude index in wrapped range [0, 360)
    ...
    return {
      ...
      forEachPoint: function (cb) {
        for (let j = 0; j < nj; j++) {
          ...
          for (let i = 0; i < ni; i++) {
            cb(
              micro.floorMod(180 + λ0 + i * Δλ, 360) - 180,
              φ0 - j * Δφ,
              row[i]
            );
          }
  ...
  function productsFor(attributes) {
    ...
    return results.filter(micro.isValue);
  }
